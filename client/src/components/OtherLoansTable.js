// Melkein identtinen PersonalLoansTable.js:n kannsa, näyttää lainaajan ID:n 
...
const LoanRow = ({loan, setReload, setLoanReturnInfo, setStatus}) => {
	...
	onHide = {() => setShowPopup(false)}/>
		...
		{`${loan.lainaus_pvm} - ${loan.arvioitu_palautus_pvm ?
		  ...
		  {/* Näyttää kuka lainaaja on ID:llä */}
		  <Col md={2}
		   style={{"display": "flex", "alignItems": "center"}}>
			{loan.lainaaja_id}
	      </Col>
...
const OtherLoansTable = ({loans, setReload}) => {
	const { t } = useTranslation();
	...
	<Col md={6} class="mt-4">
          {errorModal}
	    <Card>
		<Card.Header><h2>{t('other-loans-table_title')}</h2></Card.Header>
